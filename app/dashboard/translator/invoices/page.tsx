...
export default async function TranslatorInvoicesPage() {
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-2xl font-bold text-navy-900 mb-6">Mis facturas</h1>

      {/* Resumen */}
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl border border-navy-100 p-5">
          <p className="text-sm text-navy-500">Base imponible</p>
          <p className="text-xl font-bold text-navy-900">{totalBase.toFixed(2)} €</p>
        </div>
        <div className="bg-white rounded-xl border border-navy-100 p-5">
          <p className="text-sm text-navy-500">IVA repercutido</p>
          <p className="text-xl font-bold text-navy-900">{totalVat.toFixed(2)} €</p>
        </div>
        <div className="bg-white rounded-xl border border-navy-100 p-5">
          <p className="text-sm text-navy-500">Total facturado</p>
          <p className="text-xl font-bold text-accent-600">{totalBilled.toFixed(2)} €</p>
        </div>
      </div>

      {invoices.length === 0 ? (
        <div className="bg-white rounded-xl border border-navy-100 p-12 text-center">
          <p className="text-navy-500">No hay facturas todavía.</p>
          <p className="text-sm text-navy-400 mt-1">
            Las facturas se generan cuando un pedido se entrega o cierra.
          </p>
        </div>
      ) : (
                ...
                return (
                  <tr key={inv.id} className="hover:bg-navy-50/50">
                    <td className="px-4 py-3">
                      <Link
                        href={`/dashboard/translator/orders/${inv.orderId}`}
                    ...
                    <td className="px-4 py-3 text-navy-700">
                      {inv.order.client.name || inv.order.client.email}
                    </td>
